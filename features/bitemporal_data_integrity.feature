Feature: Bitemporal entity integrity
  ...
  Background: A DAF database with one entry for a closed rectangle
    ...
    And the database table status_report should contain rows with the following entries and no more
      | file_extract_id | table_name                    | non_positive_interval_registrering | non_positive_interval_virkning | bitemporal_entity_integrity_count | bitemporal_entity_integrity_instances | bitemporal_entity_integrity_objects |
      | 1               | Adresse                       | 0                                  | 0                              | 0                                 | 0                                     | 0                                   |
